...
| `EL-PATH` | клик по LINE/TEXT (начало) → клик (конец) → **кратчайший путь** по линиям, подсветка, длина |
...
| `EL-GRAPH-EXPORT` | топология → **Graphviz .dot** (+PNG, если установлен dot) |

### Аудит
| Команда | Описание |
|---|---|
| `EL-CHECK` | дефектоскоп: изолированные, near-miss разрывы, дубликаты текста, цепи без подписей |
| `EL-CROSSING` | **X-пересечения линий без узла** (маркеры на слое EL_CROSSING, зум) |
...
| `AW33-HTML` | спецификация → **HTML-таблица** (стили wire-table) + открытие в браузере |
...
| Команда | Описание |
|---|---|
| `EL-BOM` | спецификация компонентов: подсчёт вхождений блоков (все или по выбору) → таблица |
| `EL-TITLE` | автозаполнение штампа: лист N / всего листов / дата / обозначение по атрибутам (все вхождения блока) |
| `EL-SHEET-LIST` | **реестр листов проекта**: все DWG в папке (фоновое чтение, без открытия) → лист/имя/дата из штампов → CSV |
| `EL-XREF-LIST` | статусы внешних ссылок (OK / файл не найден / выгружен / вложенные) + CSV |
| `EL-CHECK-REPORT` | дефектоскоп + отчёт в **md/html** с таблицей сводки и деталями |
| `EL-AUTOTAG` | номера цепей C1…Cn в кружках (слой EL_TAGS) — все цепи разом |
