...
| Команда | Описание |
|---|---|
| `EL-REPORT` | **полный отчёт по чертежу** → HTML: топология, дефекты, спецификация проводов (авто-парсинг), BOM, соединения, штамп |
| `EL-PROJECT-REPORT` | **сводный отчёт по проекту**: все DWG папки (фоновое чтение) → реестр листов + KPI + провода по листам |
| `EL-REVISION-DIFF` | **что изменилось между ревизиями**: текущий чертёж vs OMNI-снап → добавленные/удалённые/изменённые провода (цвет/сечение/длина), блоки, цепи |
| `EL-CHECK-REPORT` | дефектоскоп + отчёт в **md/html** с таблицей сводки и деталями |

### Автоматизация
| Команда | Описание |
|---|---|
| `EL-JOIN` | **объединение разрозненных LINE в полилинии** (разложение сети на сегменты, контроль последовательности, опция удаления исходных) |
...
| `EL-AUTOTAG` | номера **выбранных** цепей 1, 2, 3… (без префикса), одна цепь = один номер (слой EL_TAGS) |
